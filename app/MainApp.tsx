...
import { List, MapPin, Heart, Star, ArrowUpDown } from "lucide-react";
                    ...
                    return (
                      <motion.div
                        ...
                        className={clsx(
                          ...
                          isSelected
                            ...
                            : activity.isBusinessActivity && activity.businessId && businessProfilesMap[activity.businessId]
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-1.5">
                            <button
                              ...
                              aria-label={favorites.has(activity.id) ? "Aus Favoriten entfernen" : "Zu Favoriten hinzufügen"}
                            >
                              <Heart className={clsx("h-4 w-4 transition-all", favorites.has(activity.id) && "fill-current scale-110")} />
                            </button>
                            {activity._isClosed ? (
                            ...
                            {/* Promotion Badges */}
                            {activity.promotionLevel && activity.promotionLevel !== "none" && (
                              <span className={clsx(
                                ...
                                {activity.promotionLevel === "sponsored" ? "⭐ SPONSORED" : activity.promotionLevel === "boost" ? "🚀 BOOST" : "✨ FEATURED"}
                              </span>
                            )}
                            {/* Business Activity Badge - Eindeutig für verifizierte Businesses */}
                            {activity.isBusinessActivity && activity.businessId && businessProfilesMap[activity.businessId] && (
                        ...
                        <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.08em] text-black/60">
                          ...
                          {/* Business Profile Info - Prominent für verifizierte Businesses */}
                          {activity.isBusinessActivity && activity.businessId && businessProfilesMap[activity.businessId] && (
                            <button
                              type="button"
                              onClick={(e) => {
                            ...
                            >
                              ...
                              <span className="truncate max-w-[140px]">{businessProfilesMap[activity.businessId].businessName}</span>
                              {businessProfilesMap[activity.businessId].promotionCredits > 0 && (
                                <span className="ml-1 px-1.5 py-0.5 rounded-full bg-white/20 text-[9px] font-bold">
                                  {businessProfilesMap[activity.businessId].promotionCredits} Credits
                                </span>
                              )}
                            </button>
                          )}
                          <span className="text-black/50 normal-case tracking-normal flex items-center gap-1">
                            ...
                            {activity.hostUserId ? (
                              <button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setProfileSheetUserId(activity.hostUserId!);
                                  setIsProfileSheetOpen(true);
                                }}
                        ...
                        <div className="mt-1.5 space-y-2">
                          {/* Teilnehmende Info und normale Actions */}
                          <div className="flex items-center justify-between text-[12px] text-black/55">
                            <div className="flex items-center gap-1">
                              ...
                              <span className={isJoined ? "text-brand" : ""}>
                                {participantsCount}{activity.maxParticipants ? ` / ${activity.maxParticipants}` : ""} Teilnehmende
                              </span>
                            </div>
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleShareActivity(activity.id);
                                }}
                              ...
                              <button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleReportActivity(activity.id, activity.hostUserId ?? null);
                                }}
